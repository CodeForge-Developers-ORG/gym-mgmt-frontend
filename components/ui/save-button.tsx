...
export function SaveButton({
  ...
  const text = useMemo(() => {
    switch (status) {
      ...
      case "loading": return savingText;
      case "success": return savedText;
    }
  }, [status, idleText, savingText, savedText]);
  ...
  return (
    <div className={cn("relative inline-flex group font-sans", className)}>
      <Button
        onClick={handleClick}
        className={cn(
          "relative rounded-full h-10 px-6 text-sm font-medium transition-all duration-300 min-w-[120px] disabled:opacity-100",
          status !== "idle"
        ...
        <AnimatePresence mode="wait">
          {status !== "idle" && (
            <motion.div
              ...
              className={cn(
                "flex items-center justify-center size-5 rounded-full ring-2 overflow-visible",
                status === "success"
            ...
            >
              <AnimatePresence mode="popLayout">
                {status === "loading" && (
                  <motion.div key="loader" initial={{ opacity: 1 }} animate={{ opacity: 1 }} exit={{ scale: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="absolute inset-0 flex items-center justify-center">
